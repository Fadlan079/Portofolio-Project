...
try{
    if($_SERVER['REQUEST_METHOD']== "POST"){
        ...
        $sql = $pdo->prepare("INSERT INTO users (email,username,password,role) VALUES (:email,:username, :password,:role)");
        $sql->execute([
            'email' => $email,
            'username' => $username,
            'password' => $hash,
...
<body class="bg-neutral-950 font-sans relative">
    ...
    <section class=" ml-64 p-5 relative z-20">
        <h2 class="text-2xl font-bold text-neutral-200 p-2 m-2">Data User</h2>
        ...
        <?php foreach($users as $row): ?>
            <tr class="text-neutral-200 bg-neutral-900/30 backdrop-blur-lg">
                ...
                <td class="p-3 grid grid-cols-2 gap-5">
                    <a href="../CRUD/Update.php"  class="p-1 text-emerald-500 font-bold border border-emerald-500 rounded-lg hover:text-neutral-900 hover:bg-emerald-500 transition-all duration-300"><i class="fa-solid fa-pen-to-square"></i> Edit</a>
                    <a href="../CRUD/Delete.php?id_users=<?= $row['id_users']?>" onclick="return confirm('Yakin mau Hapus User <?=htmlspecialchars($row['username'])?> dengan ID <?=htmlspecialchars($row['id_users'])?> ?')" class="p-1 text-red-500 font-bold  border border-red-500 rounded-lg hover:text-neutral-900 hover:bg-red-500 transition-all duration-300"><i class="fa-solid fa-trash"></i> Delete</a>
                </td>
            </tr>
        <?php endforeach?>
       </table>
    ...
    <div id="form" class="opacity-0 scale-95 pointer-events-none fixed z-30 inset-0 bg-neutral-950/30 backdrop-blur-xl transition-all duration-300">
        <form action="user-management.php" method="POST" class="grid grid-cols-1 bg-neutral-950/50 backdrop-blur-lg gap-7 p-5 m-auto mt-20 relative z-100 max-w-xl rounded-lg shadow-xl border border-neutral-700 text-neutral-300 font-bold">
            ...
            <div class="grid grid-cols-2 gap-5 text-center">
                <input type="submit" value="Tambah User" class="col-span-2 bg-orange-500 text-neutral-200 p-2 rounded-lg hover:bg-emerald-500 hover:text-neutral-900 transition-all duration-300">
